 ...
 */
public class PeekableQueueImpl3<E extends Comparable<E>> implements
		ExamPeekableQueue<E> {
	...
	TreeSet<E> smallThanMedian;
	TreeSet<E> biggerThanMedian;

	public PeekableQueueImpl3() {
		smallThanMedian = new TreeSet<E>();
		biggerThanMedian = new TreeSet<E>();
	}
...
//		printListz(minStack2);
		...
		if(smallThanMedian.size()>0){
			E last=smallThanMedian.last();
			if(e.compareTo(smallThanMedian.last())>0){
				Assert.assertTrue(biggerThanMedian.contains(e));
				biggerThanMedian.remove(e);
			}
			else{
				Assert.assertTrue(smallThanMedian.contains(e));
				smallThanMedian.remove(e);
			}
		}
	...
	public void enqueue(E e) {
		...
		// median
		if (data.size == 0) {
			biggerThanMedian.add(e);
		} else {
			if (!biggerThanMedian.isEmpty()) {
				if (e.compareTo(biggerThanMedian.first()) >= 0) {
					biggerThanMedian.add(e);
				} else if (e.compareTo(biggerThanMedian.first()) < 0) {
					smallThanMedian.add(e);
				} else {
					System.out.println("System error 1");
				}
			} else if (!smallThanMedian.isEmpty()) {
				if (e.compareTo(smallThanMedian.first()) <= 0) {
					smallThanMedian.add(e);
				} else if (e.compareTo(smallThanMedian.first()) > 0) {
					biggerThanMedian.add(e);
				} else {
...
//		printListz(minStack2);
	...
	private void balance() {
		while (biggerThanMedian.size() - smallThanMedian.size() > 1) {
			...
			smallThanMedian.add(biggerThanMedian.pollFirst());
		}
		while (biggerThanMedian.size() - smallThanMedian.size() < 0) {
			biggerThanMedian.add(smallThanMedian.pollLast());
		}
	}
